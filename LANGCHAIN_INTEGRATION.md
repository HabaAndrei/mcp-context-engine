...
This returns the full set of LangChain-compatible tools. The ones that matter
most to an agent's loop:

- `con_mcp_get_ready_work` - what can be started right now, best first. Lead
  with this when resuming; it is the reason to use this server over a checklist.
- `con_mcp_get_issue_stats` - board-wide counts, cheap enough to call on start.
- `con_mcp_create_epic_with_children` - record a whole breakdown atomically.
- `con_mcp_claim_issue` - take ownership before working, so two agents sharing
  a database do not duplicate effort.
- `con_mcp_close_issue` - closing is what unblocks dependent work.
- `con_mcp_add_comment` - capture why a decision was made.
- `con_mcp_get_blocked_issues` - when ready work is empty, find out why.

See the tool tables in [README.md](README.md#tools) for the complete list.
